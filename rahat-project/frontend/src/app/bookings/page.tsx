...
export default function BookingsPage() {
  ...
  const filteredBookings = sortedBookings.filter(b => {
    ...
    if (activeTab === 'UPCOMING') return b.status === 'CONFIRMED' && b.date >= today;
    if (activeTab === 'PAST') return b.status === 'CONFIRMED' && b.date < today;
    return true;
  });
    ...
    <div className="p-6 lg:p-14 w-full max-w-5xl">
      ...
      >
        <div className="glass-card p-4 flex items-center justify-between border-l-4 border-l-cyan-500">
          <div>
            <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">Total Spent</div>
        ...
        <div className="glass-card p-4 flex items-center justify-between border-l-4 border-l-green-500">
          <div>
            <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">Confirmed</div>
            <div className="text-2xl font-bold text-white">{confirmedCount} stays</div>
          </div>
        </div>
        <div className="glass-card p-4 flex items-center justify-between border-l-4 border-l-red-500">
          <div>
            <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">Cancelled</div>
      ...
      {filteredBookings.length === 0 ? (
        <div className="glass-panel p-10 text-center flex flex-col items-center justify-center min-h-[300px]">
          ...
          <p className="text-slate-400 mb-6">You don't have any {activeTab.toLowerCase()} reservations.</p>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                            ...
                            {booking.status === 'CONFIRMED' && isPast ? 'COMPLETED' : booking.status}
                          </span>
                          <span className="text-slate-500 text-sm font-mono">#{booking.id.toUpperCase()}</span>
                        </div>
